...
void RegistrationViewer::update_face_indices() {
	...
	for (int i = 0; i < (int)meshes_.size(); i++) {
		...
		for (; f_it != f_end; ++f_it) {
			indices_[i].push_back((*(fv_it = meshes_[i].cfv_iter(*f_it))).idx());
			indices_[i].push_back((*(++fv_it)).idx());
			indices_[i].push_back((*(++fv_it)).idx());
		}
	}
}
...
void RegistrationViewer::draw(const std::string& _draw_mode)
{
	...
	/*for (int i = 0; i < (int)sampled_points_1.size(); i++)
	{
		glPushMatrix();
		Vector3d pt = sampled_points_1[i];
		pt = transformations_[cur_index_].transformPoint(pt);
		glTranslatef(pt[0], pt[1], pt[2]);
		glutSolidSphere(average_vertex_distance_, 10, 10);
		glPopMatrix();
...
void RegistrationViewer::clean_mesh(Mesh& _mesh)
{
	...
	for (; f_it != _mesh.faces_end(); ++f_it)
	{
		float max_edge = 0;
		float min_edge = INF;
		Mesh::ConstFaceHalfedgeIter fh_it = _mesh.fh_iter(*f_it);
		while (fh_it.is_valid())
		{
			OpenMesh::Vec3f p = _mesh.point(_mesh.from_vertex_handle(*fh_it));
			OpenMesh::Vec3f q = _mesh.point(_mesh.to_vertex_handle(*fh_it));
...
std::vector<int> RegistrationViewer::subsample(const std::vector< Vector3d >& _pts) {
	...
	int i = 0;
	indeces.push_back(i);
	std::vector<Vector3d> out_pts;
	out_pts.push_back(_pts[i]);

	// find the smallest amount of vertices for all meshes
	int num = meshes_[0].n_vertices();
	for (int k = 0; k < meshes_.size(); k++)
	{
		if (meshes_[k].n_vertices() < num)
			num = meshes_[k].n_vertices();
	}

	int step = 2;

	//find the subsampled points
	for (auto it = _pts.begin(); it != _pts.end(), i < num - step; it += step, i += step)
	{
		bool inside_range = false;

		int window_size = 40;
		auto front = out_pts.begin();

		if (out_pts.size() < window_size)
		{
			front = out_pts.begin();
		}
		else
		{
			front = out_pts.begin() + out_pts.size() - window_size;
		}

		// iterate through to gain the subsampled vertices
		for (auto iterTarget = front; iterTarget != out_pts.end(); iterTarget++)
		{
			if (length(*it - *iterTarget) < subsampleRadius)
				inside_range = true;
		}
...
void RegistrationViewer::calculate_correspondences(
	...
	for (int index = 0; index < srcCandidateNormals.size(); index++)
	{
		...
		auto angle = 2 * asin(length_long_edge / 2) * 180 / M_PI;

		if (!(src_target_dis2[index] > distMedianThresh) && !(angle > normalCompatabilityThresh))
		{
			_src.push_back(srcCandidatePts[index]);
			_target.push_back(targetCandidatePts[index]);
			_target_normals.push_back(targetCandidateNormals[index]);
		}
	}
